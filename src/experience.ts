export class ExperienceSystem {
    ...
    private readonly comboTimeout: number = 3000; // 3 seconds for more dynamic gameplay
    private readonly comboMultipliers: number[] = [1, 1.2, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0]; // Extended combo system
    private consecutiveTyping: number = 0;
    private perfectComboStreak: number = 0;
     ...
     */
    processTyping(typedChars: number): number {
        const now = Date.now();
    ...
    static getExpForLevel(level: number): number {
        if (level <= 4) return 100; // Levels 1-4: 0-99
        if (level <= 14) return 500; // Levels 5-14: 100-499
        if (level <= 29) return 1500; // Levels 15-29: 500-1499
        if (level <= 49) return 3000; // Levels 30-49: 1500-2999
     ...
     */
    static getLevelFromExp(experience: number): number {
        if (experience < 100) return Math.floor(experience / 25) + 1; // Levels 1-4
        if (experience < 500) return Math.floor((experience - 100) / 40) + 5; // Levels 5-14
        if (experience < 1500) return Math.floor((experience - 500) / 67) + 15; // Levels 15-29
        if (experience < 3000) return Math.floor((experience - 1500) / 75) + 30; // Levels 30-49
